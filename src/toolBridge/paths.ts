...
/**
 * Browser File System Access fallback prefix. Must stay in sync with
 * {@link ../localWorkspace/files.ts}.
 */
...
/**
 * Raised by {@link resolveAllowedPath} when a requested path cannot be safely
 * resolved against the workspace roots in scope for a tool call. The orchestrator
 * surfaces these as structured tool errors rather than letting them surface as
 ...
 *   attempts ("/root/../../etc/passwd") fail the comparison.
 * - Browser workspace URLs (`browser-folder://...`) are compared by exact
 *   prefix (no case folding) since their segments are URL-encoded.
 * - Desktop paths are compared case-insensitively (matches the existing
 *   workspace conventions and tolerates Windows drive-letter casing).
 *
 * Symlinks are NOT followed. The tool bridge does not currently shell out
 * to `realpath` from JS — a symlinked file inside a root will resolve as
 * if it lives there. Treat this as a known limitation: hard-gated tools
 * (terminal, destructive, etc.) still go through the approval flow.
 */
...
/**
 * Pure form of {@link resolveAllowedPath} that returns a discriminated union
 * instead of throwing. Useful from tool execute() bodies that want to convert
 * a structured failure into a `ToolExecutionResult` directly.
 */
...
function isAbsoluteLikePath(path: string): boolean {
  ...
  if (/^[a-zA-Z]:[\\/]/.test(path)) {
    return true; // Windows drive letter with separator
  }
  return false;
}
...
function collapseSegments(body: string, separator: string): string {
  ...
  for (const segment of segments) {
    ...
    if (segment === "..") {
      ...
      // Going above the prefix is a no-op; the membership check will catch it.
      continue;
    }
    stack.push(segment);
  }
