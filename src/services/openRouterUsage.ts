...
import { createOpenRouterStreamRequestBody, modelForMessages, type OpenRouterUsage } from "./openRouterClient";
import type { ChatAttachment, ChatMessage, ChatSummary } from "../types/chat";
import type { ProviderSettings } from "../types/settings";

interface OpenRouterContextUsageInput {
  chat: ChatSummary;
...
interface OpenRouterProviderContextUsageInput {
  contextWindowTokens: number;
...
export function estimateOpenRouterContextWindowUsage({
  chat,
  contextWindowTokens,
  draftAttachments,
  draftContent,
  settings,
  source,
}: OpenRouterContextUsageInput): ContextWindowUsage {
  ...
  return estimateOpenRouterUsageFromMessages({
...
export function estimateOpenRouterProviderContextUsage({
  contextWindowTokens,
  messages,
  settings,
  source,
}: OpenRouterProviderContextUsageInput): ContextWindowUsage {
  return estimateOpenRouterUsageFromMessages({
...
function estimateOpenRouterUsageFromMessages({
  chatMessageCount,
  contextWindowTokens,
  draftCount,
  messages,
  ...
  const requestOverheadTokens = Math.max(serializedBodyTokens - systemTokens - messageTokens - draftTokens, estimateOpenRouterControlTokens(body));
...
export function applyOpenRouterUsageToContextEstimate(estimate: ContextWindowUsage, usage: OpenRouterUsage | undefined): ContextWindowUsage {
...
function estimateOpenRouterControlTokens(body: ReturnType<typeof createOpenRouterStreamRequestBody>) {
  const { messages: _messages, ...controlBody } = body;
