...
| macOS arm64/x64 | Release workflow wired, Apple signing secrets pending, native launch unverified | Frontend platform detection, Mac shortcut labels, left-side traffic-light window controls, Keychain-backed secure storage, app/DMG build scripts, macOS Rust CI, native desktop command-path coverage, 9Router native tool-path checks, and GitHub Actions release jobs are in place. Trusted macOS artifact publication is skipped until Apple Developer signing/notarization secrets are configured, and a maintainer still needs to run the packaged app and complete the native QA checklist before this becomes verified support. |
...
- The GitHub `Release` workflow builds Windows first, runs macOS release checks and publishes macOS artifacts only when Apple Developer signing/notarization secrets exist, then builds Linux x64 so updater metadata is merged without parallel upload races.
