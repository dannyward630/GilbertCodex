...
The tool system is the heart of Gilbert, and the next rebuild is aimed at making it stricter, faster, and less surprising.

- Replace permissive XML recovery paths with a smaller schema-driven protocol.
- Generate model-facing tool instructions from one authoritative tool schema.
- Improve argument validation so malformed calls become clear retryable errors instead of silent wrong behavior.
- Batch safe tool calls more confidently while keeping destructive actions behind review.
- Persist tool call IDs/results cleanly so native provider tools can return later without protocol leaks.
...
- Make voice mode work naturally with planning, web search, files, and coding tasks.

### Sidebar And Workspace Layout Redo

The navigation needs to scale better as users add more projects, chats, tools, and integrations.

- Redesign the side menu around fast switching between chats, projects, Toolbox, Workflows, Settings, and integrations.
- Improve pinned chats, recent workspaces, search, collapsed density, and section ordering.
- Make project context clearer before an agent writes files or runs tools.
...
- Group related tool calls into readable runs.
...
- Model capability badges for tools, images, voice, context length, reasoning, and speed.
...
- Safer fallback behavior when hosted routes ignore tools or return malformed output.
...
- Image generation and image editing as real chat tools with saved artifacts.
...
- More reusable `workflow_run` recipes for repo audits, plan-patch-verify loops, release prep, and research-backed fixes.
...
- Add focused tests around provider request creation, tool-call parsing, chat utilities, and storage normalization.
...
- Add provider compatibility notes when a model works well or fails reliably with tools.
